...
export default class Datepicker {
	...
	open(date) {
		...
		date =
			date || typeof this._opts.openOn === 'function' ? this._opts.openOn.call(this) : this._opts.openOn || this._month;
	...
	toggle() {
		if (this._isOpen) {
			...
			if (this._opts?.isOpenDisabled() === true) return;
			this.open();
		}
	}
	 ...
	 */
	render() {
		let { ranged, time, onRender, getFirstMonthByCurrent } = this._opts;
		// don't render
		if (this._noRender || !this._renderers) return;
		// avoid duplicate calls to getData
		let renderCache = {};
		let getData;
		if (getFirstMonthByCurrent) {
			getData = (i) =>
				renderCache[i + getFirstMonthByCurrent()] ||
				(renderCache[i + getFirstMonthByCurrent()] = this.getData(i + getFirstMonthByCurrent()));
		} else {
			getData = (i) => renderCache[i] || (renderCache[i] = this.getData(i));
		}
	...
	getData(index = 0) {
		...
		if (index === 0) {
			this.constructor.defaults.selectedFirstDay = null;
			this.constructor.defaults.selectedLastDay = null;
		}
		...
		for (let i = 0; i < dayCount; i++) {
			...
			if (isSelected) {
				if (
					(this.constructor.defaults.selectedFirstDay?.getDate() === day?.getDate() &&
						this.constructor.defaults.selectedFirstDay?.getMonth() === day?.getMonth()) ||
					this.constructor.defaults.selectedFirstDay === null
				) {
					classNames.push(selectedFirst);
					this.constructor.defaults.selectedFirstDay = day;
				}

				classNames.push(selectedClass);
			} else if (this.constructor.defaults.selectedFirstDay && !this.constructor.defaults.selectedLastDay) {
				this.constructor.defaults.selectedLastDay = days[i - 1]?._date;
				days[i - 1]?.classNames.push(selectedLast);
			}
